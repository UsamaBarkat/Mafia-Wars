...
Phase 1 COMPLETE — built, verified, pushed to GitHub (private repo Mafia-Wars), and deployed live on Vercel. Next: Phase 2 (online multiplayer) — begins with a research phase before any spec.

## Known visual issue

On mobile (real phone), the Home screen content is vertically centered with a large empty gap at the top, forcing a slight scroll and showing screens taller than the viewport. Fix in next session: make screens fit within the mobile viewport height (e.g. min-h-screen / dvh with balanced vertical layout) so no scrolling is needed on phones. Visual only — game logic is unaffected.
